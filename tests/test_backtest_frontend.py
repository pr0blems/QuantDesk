...
def test_backtest_workbench_switches_to_martingale_basket_profile() -> None:
    ...
    for contract in (
        ...
        'this.calculatorEffectiveAtrFactor()',
        'market_state_factor_multiplier',
...
def test_backtest_supports_parameter_profiles_and_multiple_symbols() -> None:
    ...
    for contract in (
        ...
        "reloadSavedParameterProfiles()",
        "loadSelectedParameterProfiles({ force: true })",
        'preferred_profile: payload.preferred_profile',
        "尚未保存最优参数，当前使用策略内置默认值",
        "专有最优参数（优先于策略默认配置）",
...
def test_history_reuses_current_result_card_and_replay_ui() -> None:
    script = (ROOT / "web/src/controllers/backtest.js").read_text(encoding="utf-8")
    stylesheet = (ROOT / "web/public/assets/backtest.css").read_text(encoding="utf-8")

    for contract in (
        "buildResultSummaryCard(item, index, options = {})",
        "this.buildResultSummaryCard(item, index",
        "this.buildResultSummaryCard(resultItem, index",
    ...
    load_run = script.split("  async loadRun(id, button) {", 1)[1].split("\n  renderResult(", 1)[0]
    assert "this.renderResult(detail);" not in load_run
    assert ".history-result-card.loading" in stylesheet
    assert ".history-list { min-height: 210px; max-height: calc(100vh - 230px); display: grid; grid-template-columns: minmax(0, 1fr)" in stylesheet
