...
_UNAVAILABLE_DATABASE_URL = (
    "mysql+pymysql://test:test@127.0.0.1:3306/quantdesk_test_unavailable"
)
...
def _set_test_environment(database_url: URL | str) -> None:
    values = {
        ...
        "DB_SSL_VERIFY_IDENTITY": os.environ.get(
            "QUANTDESK_TEST_DB_SSL_VERIFY_IDENTITY", "false"
        ),
...
def _test_connect_args() -> dict[str, object]:
    if os.environ.get("QUANTDESK_TEST_DB_SSL_REQUIRED", "false").lower() not in {
        ...
        return {}
    verify_identity = os.environ.get(
        "QUANTDESK_TEST_DB_SSL_VERIFY_IDENTITY", "false"
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    if admin_value and database_value:
        raise RuntimeError(
            f"set only one of {TEST_ADMIN_URL_ENV} or {TEST_DATABASE_URL_ENV}"
        )
