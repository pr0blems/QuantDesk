...
@router.post("/monitor/opportunities/{opportunity_public_id}/shadow")
def create_shadow_intent(
    opportunity_public_id: str,
    payload: ShadowOpportunityRequest,
    request: Request,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
) -> dict[str, Any]:
    """Create a risk-audited simulated intent; this endpoint can never submit to Binance."""
    _require_expected_user(request, user)
    opportunity = db.scalar(
        select(MarketOpportunity).where(
            MarketOpportunity.public_id == opportunity_public_id,
            MarketOpportunity.current_marker == 1,
        )
    )
    if opportunity is None or opportunity.direction not in {"long", "short"}:
        raise HTTPException(status_code=404, detail="actionable market opportunity not found")
    ticker = (
        db.execute(
            text("SELECT price,ts FROM ticker WHERE symbol=:symbol"), {"symbol": opportunity.symbol}
        )
        .mappings()
        .first()
    )
    if ticker is None or Decimal(str(ticker["price"])) <= 0:
        raise HTTPException(status_code=409, detail="fresh market price is unavailable")
    now_ts = int(datetime.now(UTC).timestamp())
    market_age = max(0, now_ts - int(ticker["ts"] or 0))
    kill_setting = db.get(AdminSetting, "execution_kill_switch")
    kill_active = bool(
        kill_setting
        and isinstance(kill_setting.value_json, dict)
        and kill_setting.value_json.get("enabled")
    )
    account = db.scalar(
        select(ExchangeAccount).where(
            ExchangeAccount.user_id == user.id,
            ExchangeAccount.name == "QuantDesk Shadow",
        )
    )
    if account is None:
        account = ExchangeAccount(
            user_id=user.id,
            name="QuantDesk Shadow",
            venue="binance",
    ...
    quantity = payload.notional_usdt / price
    reasons: list[str] = []
    if kill_active:
        reasons.append("GLOBAL_KILL_SWITCH")
    if market_age > 10:
        reasons.append("MARKET_DATA_STALE")
    if payload.notional_usdt > Decimal("1000"):
        reasons.append("ORDER_NOTIONAL_LIMIT")
    decision = RiskDecisionRecord(
        user_id=user.id,
        exchange_account_id=account.id,
        subject_type="market_opportunity",
        ...
        exposure_json={
            "order_notional": str(payload.notional_usdt),
    ...
    intent = OrderIntent(
        ...
        idempotency_key=f"shadow-opportunity:{opportunity.public_id}",
        symbol=opportunity.symbol,
        ...
        leverage=payload.leverage,
        reduce_only=False,
        state="risk_rejected" if reasons else "approved",
    ...
    _audit(
        db,
        request,
        "execution.shadow_intent.create",
        user.id,
        "market_opportunity",
        opportunity.public_id,
        {"approved": not reasons, "reason_codes": reasons},
    )
    try:
        db.commit()
    except IntegrityError:
        db.rollback()
        raise HTTPException(status_code=409, detail="shadow intent already exists") from None
    return {
        "id": intent.public_id,
        "mode": "shadow",
        "state": intent.state,
