...
def test_contract_matrix_emphasizes_ai_direction_and_live_price_moves() -> None:
    ...
    assert "多周期多空预测" in script
    assert "未校准" in script
    assert 'const order = ["5m", "15m", "1h", "2h", "4h"]' in script
    assert "LightGBM 校准模型" in script
...
def test_contract_matrix_is_ai_first_and_every_visible_indicator_sorts() -> None:
    ...
    assert "monitor.js?v=20260806-17" in index
    assert "monitor.css?v=20260806-17" in script
    ...
    assert "underlying_quote" in script
    assert "spreadAlertStates" in script
    assert "maybeNotifySpreadAlert" in script
