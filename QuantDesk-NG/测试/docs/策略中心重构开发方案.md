...
## 25. 2026-08-04 第二阶段实施记录：机会层

本轮已完成阶段 5 的首个可运行闭环：

- 新增共享市场特征扫描器 `core_market_features v1`，统一从已收盘的 15m、1h、4h K 线计算 EMA、ATR、ADX、DI、Donchian、成交量比、动量和数据完整度。
- 新增可解释机会扫描器 `market_bias_v1`，输出 `long`、`short`、`neutral` 三种方向，以及 `detected`、`watching`、`confirmed`、`expired` 生命周期状态。
- 每个机会包含稳定去重键、质量分、发现 K 线、失效 K 线、各周期偏向、条件布尔值和原因代码；同一收盘 K 线重复扫描不会新增重复记录。
- 扫描器接入合约 K 线采集循环：全部周期更新完成后执行一次全市场扫描，单个合约数据不足不会阻塞其他合约和旧评分链路。
- 合约监控保留旧综合评分作为指标参考，同时优先展示真正的市场机会方向、质量、证据、发现时间和有效期；多头使用绿色、空头使用红色、中性使用白色。
- 新增机会查询 API，返回当前用户匹配的已发布完整策略；完整策略后续生成的模拟盘信号会关联当时有效的公共市场机会 ID。
- 新增 `user_opportunity_states` 表，主键为 `user_id + opportunity_id`；一个用户关注或忽略机会不会改变其他用户看到的公共机会状态。
- 用户关注机会时只给该用户生成提醒；关注、忽略和清除偏好均写入审计日志。
- 实盘自动执行未启用，本阶段只完成市场发现、解释、用户偏好和模拟盘信号关联。

真实数据验收：

- MySQL 迁移版本已升级为 `0017_opportunity_states`，新表及全部字段中文注释已通过 `information_schema` 核验。
- 当前配置 150 个合约；144 个具有完整三周期历史并成功扫描，6 个因少于 55 根有效历史 K 线安全跳过。
- 首轮落库 432 条特征快照和 144 条机会：90 个多头、29 个空头、25 个中性。
- 合约监控总览成功关联 144 个最新机会；用户 1 的完整策略可分别匹配多头和空头机会，中性机会不错误匹配入场策略。

阶段 5 后续增强项：

- 将用户“关注某类机会”的规则从单事件偏好扩展为按标的、方向、质量阈值和策略的订阅规则。
- 增加机会状态变化推送，避免只有用户主动点击关注时生成提醒。
- 增加机会历史、命中率和从机会到策略信号、模拟成交的转化漏斗。
- 对 6 个历史不足的合约继续采集数据，满足最小样本后自动进入扫描。
