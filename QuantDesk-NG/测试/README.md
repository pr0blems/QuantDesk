...
面向 Binance TradFi 永续合约的多用户量化研究、机会发现、策略验证和受控执行平台。
NG 版本将 Web API 与行情/舆情/模拟盘 Worker 解耦，并建立幂等订单、事件、成交、风控和 Outbox 领域基础。
实盘执行仍被配置门禁锁定。

新架构和启动方法见 [NG 架构与运行说明](docs/NG架构与运行说明.md)。
