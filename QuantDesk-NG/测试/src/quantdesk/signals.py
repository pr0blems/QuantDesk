...
def score_klines(kl):
    ...
    if hist is not None:
        if hist > 0 and dif > 0:
            ...
            vote, pts = 1, 0.5
            reason = (
                f"DIF({dif:.4g}) 仍在零轴下方，但柱体转正({hist:.4g})，下跌动能收敛、有金叉迹象"
            )
            ...
            vote, pts = -1, 0.5
            reason = (
                f"DIF({dif:.4g}) 仍在零轴上方，但柱体转负({hist:.4g})，上涨动能衰减、有死叉迹象"
            )
    ...
    if st is not None:
        vote, pts = st, 1.0
        reason = (
            "SuperTrend(10,3) 为多头方向，价格运行在趋势线上方，趋势止损线提供下方支撑"
            if st > 0
            else "SuperTrend(10,3) 为空头方向，价格运行在趋势线下方，趋势止损线构成上方压力"
        )
        factors.append(
            dict(key="supertrend", name="SuperTrend", vote=vote, points=pts, reason=reason)
        )
    else:
    ...
    if n >= 20:
        ...
        width = (bu - bl) / bm * 100
        squeeze = (
            "；⚠️ 带宽仅 " + f"{width:.1f}%" + "，布林带明显收窄，变盘临近，重点盯突破方向"
            if width < 4
            else ""
        )
        if pctb > 1.0:
            vote, pts = -1, 0.7
            reason = (
                f"%B={pctb:.2f}，价格突破上轨({bu:.4g})，短线偏离均值过远，警惕冲高回落{squeeze}"
            )
        elif pctb >= 0.8:
            vote, pts = 1, 0.6
            ...
            vote, pts = -1, 0.6
            reason = (
                f"%B={pctb:.2f}，价格贴近下轨({bl:.4g})，处于弱势区（带宽 {width:.1f}%）{squeeze}"
            )
        else:
            vote, pts = 1, 0.7
    ...
    if a is not None and c > 0:
        atr_pct = a / c * 100
        hist_a = [
            ind.atr(highs[:i], lows[:i], closes[:i]) for i in range(len(closes) - 20, len(closes))
        ]
        ...
        reason = (
            f"ATR(14)={a:.4g}，约占现价 {atr_pct:.2f}%（{regime}，为近20期均值的 {rel:.1f} 倍）；"
    ...
    for f in factors:
        ...
        f["weight"] = w
        f["contribution"] = (
            round(f["vote"] * f["points"] * w / total_w * 100, 1) if (total_w and w) else 0
        )
