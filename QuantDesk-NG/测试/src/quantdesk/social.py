...
_BASE_TYPE = {
    s["symbol"].replace("USDT", "").replace("USD1", ""): s.get("underlyingType", "")
...
def st_symbol_sentiment(base):
    d = _get_json(
        f"https://api.stocktwits.com/api/2/streams/symbol/{urllib.parse.quote(base)}.json"
    )
    bull = bear = 0
...
def _covered(base):
    return (
        _BASE_TYPE.get(base) in _COVERED_TYPES
        and base not in _SKIP_BASES
        and base not in _unsupported
    )
...
def priority_symbols():
    ...
    watch = []
    for row in store.query(
        "SELECT monitor_watchlist FROM users WHERE is_active=1 AND monitor_watchlist IS NOT NULL"
    ):
...
def social_loop(stop_event=None):
    while stop_event is None or not stop_event.is_set():
        if store.collector_paused("social"):
            if stop_event is not None and stop_event.wait(15):
                break
