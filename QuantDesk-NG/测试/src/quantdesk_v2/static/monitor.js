class ContractMonitor extends HTMLElement {
  ...
  updateWatchlistUi() {
    const button = this.q("#btn-clear-watchlist");
    if (!button) return;
    button.disabled = this.state.watchlist.size === 0;
    button.textContent = `清除自选 · ${this.state.watchlist.size}`;
  }
  ...
  async syncPositionsToMonitor() {
    ...
    try {
      // The account endpoint reads the current futures positions only.  The
      // server adds those symbols to the monitor and refreshes market coverage.
  ...
  renderGrid() {
    ...
    items.sort((left, right) => {
      // The local list includes contracts synchronized from Binance positions
      // and open orders. Keep that working set ahead of the market list, then
      // apply the selected metric inside each group.
    ...
    const rows = items.map((item) => {
      ...
      const scoreClass = this.signalTone(item.score);
      ...
      const mainRow = `<tr class="matrix-row matrix-${ai.tone} ${moveClass}" data-symbol="${this.escape(item.symbol)}">
        ...
        <td class="score ${scoreClass}">${this.formatScore(item.score)}</td>
