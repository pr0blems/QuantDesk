...
class BacktestRepository:
    ...
    def run_full_strategy(
        ...
        for role, timeframe in spec["timeframes"].items():
            ...
            candles, timeframe_quality = _clean_candles(rows, series["scale"], timeframe)
        ...
        close_times = {
            timeframe: [candle.ts + int(_timeframe_seconds(timeframe) or 0) for candle in candles]
...
def _strategy_window_sizes(spec: dict[str, Any]) -> dict[str, int]:
    ...
    return {
        ...
        "setup": max(
            int(parameters["setup_ema_period"]) + int(parameters["setup_lookback_bars"]) + 10,
            int(parameters["setup_atr_period"]) + int(parameters["setup_lookback_bars"]) + 10,
        ),
        "trigger": max(
