...
def test_ai_model_frontend_uses_user_scoped_crud_contract_without_key_echo() -> None:
    ...
    payload = script[
        script.index(
            "const payload = {", script.index("async function saveAiModelConfig")
        ) : script.index(
            "if (apiKey) payload.api_key", script.index("async function saveAiModelConfig")
        )
    ]
    assert "base_url" not in payload

    renderer = script[
        script.index("function renderAiModelConfigs") : script.index(
            "function renderAiModelProviderOptions"
        )
    ]
