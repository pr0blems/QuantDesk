...
def test_full_strategy_caps_account_risk_and_uses_declared_reward_risk() -> None:
    account = _account()
    ...
    stop, target = paper._exit_levels(100, 1, 2, config)

    assert config["leverage"] == 5
    assert config["max_positions"] == 5
    assert config["risk_per_trade_pct"] == pytest.approx(0.5)
    assert config["max_margin_pct"] == pytest.approx(10)
    assert config["exit_on_regime_break"] is True
    assert stop == pytest.approx(97)
...
def test_full_strategy_signal_reads_declared_timeframes(
    ...
    monkeypatch.setattr(
        ...
        lambda account, symbol, spec, decision: (
            recorded.append((symbol, decision.decision)) or True
        ),
    )
    monkeypatch.setattr(
        paper,
...
def test_open_position_always_persists_take_profit(
    ...
    monkeypatch.setattr(paper, "_equity", lambda account, prices, positions: (10_000.0, 0.0))
    monkeypatch.setattr(
        paper,
        "_set_balance",
    ...
    opened = paper._open_position(account, "TESTUSDT", side, 100.0, 2.0, ["4h signal"], [], 1_000)
...
def test_full_strategy_position_size_is_capped_by_stop_risk(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    account = _account()
    account["strategy_snapshot_json"] = {
    ...
    monkeypatch.setattr(paper, "_prices", lambda: {"TESTUSDT": 100.0})
    monkeypatch.setattr(paper, "_equity", lambda account, prices, positions: (10_000.0, 0.0))
    monkeypatch.setattr(paper, "_set_balance", lambda account, balance: None)
    monkeypatch.setattr(
        paper.store,
        "execute",
        lambda sql, params=(): writes.append((sql, tuple(params))),
    ...
    assert paper._open_position(account, "TESTUSDT", 1, 100, 2, [], [], 1_000) is True

    params = next(params for sql, params in writes if "INSERT INTO paper_positions" in sql)
    quantity, entry, leverage, stop = params[4], params[5], params[7], params[8]
    assert leverage == 5
    assert quantity * abs(entry - stop) == pytest.approx(50)
...
def test_zero_target_is_repaired_and_closes_on_same_tick(
    ...
    monkeypatch.setattr(paper, "_strategy_signal", lambda account, symbol: (0, 2.0, [], 800))
...
@pytest.mark.parametrize(
    ("side", "low", "high", "expected_price"),
    [(1, 96.5, 101, 97), (-1, 99, 103.5, 103)],
)
def test_transient_websocket_crossing_triggers_stop_after_price_recovers(
    monkeypatch: pytest.MonkeyPatch,
    side: int,
    low: float,
    high: float,
    ...
    position = {
        ...
        "avg_entry": 100,
        "margin": 50,
        "stop": 97 if side > 0 else 103,
        "target": 110 if side > 0 else 90,
        "atr_entry": 2,
        "liq_price": 80 if side > 0 else 120,
        "funding_acc": 0,
        "opened_ts": 900,
        "protection_started_at_ms": 900_000,
    }
    closes: list[tuple[float, str]] = []
    monkeypatch.setattr(paper, "_positions", lambda account: [position])
    monkeypatch.setattr(paper, "tradfi_symbols", lambda: [])
    ...
    monkeypatch.setattr(
        paper,
        "_close_position",
        lambda account, position, price, reason, now: closes.append((price, reason)) or True,
    )
    ...
    assert closes == [(expected_price, "stop_loss")]


def test_extreme_before_position_open_does_not_trigger_stop() -> None:
    position = {
        "side": 1,
        "opened_ts": 900,
        "protection_started_at_ms": 950_000,
        "stop": 97,
        "target": 110,
        "liq_price": 80,
    }

    reason, price = paper._protective_exit(
        position,
        100,
        {"low": 96, "low_time": 949_999, "high": 101, "high_time": 949_999},
    )
...
def test_take_profit_does_not_reopen_the_same_symbol_on_the_same_tick(
    ...
    monkeypatch.setattr(paper, "_positions", lambda account: [] if state["closed"] else [position])
...
def test_paused_accounts_keep_managing_positions_without_opening_new_ones(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    active = {**_account(), "id": 11, "status": "active"}
    paused = {**_account(), "id": 12, "status": "paused"}
    ticked: list[tuple[int, bool]] = []
    ...
    monkeypatch.setattr(paper, "_market_extremes", lambda now_ms: {})

    def tick_account(account, prices, now, extremes, *, allow_entries=True):
        ticked.append((account["id"], allow_entries))

    monkeypatch.setattr(paper, "_tick_account", tick_account)

    paper.tick()

    assert ticked == [(11, True), (12, False)]
