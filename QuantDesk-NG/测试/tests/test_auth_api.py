...
def test_login_page_and_navigation_shell_are_served(mysql_test_engine: Engine) -> None:
    ...
    with client:
        ...
        assert '<html lang="zh-CN" class="auth-booting" data-theme="dark">' in page.text
        assert 'id="auth-boot"' in page.text
        assert "正在恢复登录状态" in page.text
        assert "/assets/style.css?v=20260804-7" in page.text
        assert "/assets/terminal.css?v=20260804-3" in page.text
        assert "/assets/app.js?v=20260804-7" in page.text
        assert page.text.index("/assets/backtest.js") < page.text.index(
            "/assets/app.js?v=20260804-7"
        )
        assert 'id="login-page"' in page.text
        assert 'id="sidebar"' in page.text
        assert 'id="login-theme-toggle"' in page.text
        ...
        assert "performance.account?.currency" in script.text
        ...
        background_health = script.text.index(
            'api("/api/v2/health").catch(() => null)', dashboard_loader
        )
