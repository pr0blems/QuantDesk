...
def test_system_catalog_has_full_strategy_and_all_legacy_defaults(session: Session) -> None:
    ...
    assert "2.5×ATR 止盈" in paper_template.description
...
def test_revision_relationship_and_catalog_serialization(session: Session) -> None:
    ...
    strategy = ensure_user_default_strategies(session, user.id)[1]
    strategy.version = 2
...
def test_strategy_migration_follows_quantity_precision_revision() -> None:
    ...
    assert [item[1] for item in module.SEED_TEMPLATES] == EXPECTED_NAMES[1:-1]
