...
class FakePaperRepository:
    ...
    def paper(self, user_id: int, account_id: int, timezone_offset_minutes: int = 0) -> dict:
    ...
    def paper_performance(
        ...
        self.performance_calls.append((user_id, account_id, month, timezone_offset_minutes))
        return {
...
def test_paper_endpoints_are_account_scoped_and_tenant_isolated(
    ...
    with client:
        ...
        strategy_id = client.get("/api/v2/strategies", headers=alice_headers).json()["items"][0][
            "id"
        ]
        created = client.post(
        ...
        denied = client.get(f"/api/v2/paper?account_id={public_account_id}", headers=bob_headers)
