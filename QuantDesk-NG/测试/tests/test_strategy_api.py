...
def test_first_login_copies_all_defaults_and_list_is_tenant_isolated(
    ...
    with client:
        ...
        paper_strategy = next(
            ...
            if item["source_template_key"] == "paper_multifactor_atr_v1"
        )
        assert paper_strategy["risk_defaults"]["take_profit_pct"] == 5
        assert "2.5×ATR 止盈" in paper_strategy["description"]
...
def test_ai_preview_prefers_current_users_enabled_default_model(
    ...
    with client:
        ...
        alice_strategy = client.get("/api/v2/strategies", headers=alice_headers).json()["items"][0]
