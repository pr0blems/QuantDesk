...
def _extend_strategy_revisions() -> None:
    ...
    op.add_column(
        "strategy_revisions",
        sa.Column("validation_json", sa.JSON(), comment="发布时的静态校验、数据依赖和风险提示"),
    )
    op.add_column(
        "strategy_revisions",
...
def _create_market_feature_snapshots() -> None:
    op.create_table(
        ...
        sa.Column("bar_open_time", BIGINT, nullable=False, comment="对应已收盘 K 线开盘时间戳"),
        sa.Column("feature_set_key", sa.String(64), nullable=False, comment="标准特征集合稳定标识"),
        sa.Column(
...
def _create_strategy_deployments() -> None:
    op.create_table(
        ...
        sa.Column(
            "updated_at", sa.DateTime(), nullable=False, comment="策略部署最后更新时间（UTC）"
        ),
        sa.CheckConstraint(
...
def _create_strategy_signals() -> None:
    op.create_table(
        ...
        sa.Column(
            "evidence_json", sa.JSON(), nullable=False, comment="参与策略决策的指标和条件证据"
        ),
...
def downgrade() -> None:
    ...
    op.drop_constraint("uq_strategy_revisions_id_user_id", "strategy_revisions", type_="unique")
    for column in (
        "published_at",
        "validation_json",
        "spec_hash",
