class AiMonitorDashboard extends window.QuantDeskPageController {
  ...
  toggleOpportunityDetails(button) {
    ...
    button.dataset.state = nextExpanded ? "expanded" : "collapsed";
    button.setAttribute("aria-label", `${nextExpanded ? "收起" : "展开"} ${card.querySelector(".opportunity-symbol")?.textContent?.trim() || "机会"}详情`);
    const label = document.createElement("span");
    label.textContent = nextExpanded ? "收起详情" : "展开详情";
    button.replaceChildren(label);
    card.querySelectorAll(".opportunity-details-shell").forEach((details) => {
  ...
  renderMacroImpact() {
    ...
    target.innerHTML = `<section class="macro-policy-hero ${policyTone}">
      ...
      <div class="macro-policy-metrics"><article><span>入场门槛调整</span><b>${Number(policy.threshold_delta || 0) >= 0 ? "+" : ""}${numberOrDash(policy.threshold_delta, 0)} 分</b></article><article><span>多头仓位上限</span><b>${numberOrDash(Number(policy.long_position_multiplier) * 100, 0)}%</b></article><article><span>空头仓位上限</span><b>${numberOrDash(Number(policy.short_position_multiplier) * 100, 0)}%</b></article><article><span>顺势多单</span><b>${policy.pause_new_trend_longs ? "暂停新增" : "允许"}</b></article></div>
    </section>
  ...
  patchOpportunityCards(target, markup) {
    ...
    const patchKeyedSections = (currentCard, nextCard) => {
      ...
      nextSections.forEach((nextSection, key) => {
        ...
        if (!changed) return;
        if (key === "header") {
          // 行情流会频繁改变报价与评分。更新身份信息，但保留两个顶部指标按钮的 DOM 身份，
          // 否则按钮会在 pointerdown/click 之间被替换，表现为偶发“点击无反应”。
          syncAttributes(currentSection, nextSection);
          const currentIdentity = currentSection.querySelector(".opportunity-identity");
          const nextIdentity = nextSection.querySelector(".opportunity-identity");
          if (currentIdentity && nextIdentity && currentIdentity.innerHTML !== nextIdentity.innerHTML) {
            currentIdentity.replaceChildren(...nextIdentity.childNodes);
          }
  ...
  renderOpportunities() {
    ...
    const markup = settlementGuide + visibleOpportunities.map((item) => {
      ...
      const detailControl = `<button class="opportunity-detail-toggle" type="button" data-toggle-opportunity-details="${this.escape(item.id)}" data-state="${expanded ? "expanded" : "collapsed"}" aria-expanded="${expanded}" aria-label="${expanded ? "收起" : "展开"} ${this.escape(item.symbol)} 详情"><span>${expanded ? "收起详情" : "展开详情"}</span></button>`;
      const liveSummaryActions = manualFollowControl
        ? `<div class="virtual-position-action">${manualFollowControl}${conclusionControl}${detailControl}</div>`
        : "";
      ...
      const targetStateLabel = position.target_state === "take_profit_reached"
        ...
        ? "预测已结算"
        : "持仓观察中";
      const positionPanel = triggeredPosition ? `<section class="virtual-position ${positionTone} direction-${directionClass}" data-patch-key="position" aria-label="持仓实时盈亏">
        <div class="virtual-position-head"><div class="virtual-position-title"><span>POSITION · ${this.state.displayLeverage}X DISPLAY</span><strong>${directionLabel} · ${targetStateLabel}</strong><small>触发 ${this.formatDate(entryTime)} · 有效至 ${signalEnd} · 不会发送真实订单</small></div>${liveSummaryActions}</div>
      ...
      const candidatePositionPanel = candidateSummaryVisible ? `<section class="virtual-position candidate-position direction-${directionClass}" data-patch-key="candidate-position" aria-label="候选信号摘要">
        <div class="virtual-position-head"><div class="virtual-position-title"><span>SIGNAL · ${this.escape(item.timeframe)} CANDIDATE</span><strong>${directionLabel} · ${this.escape(entryState.label)}</strong><small>发现 ${signalStart} · 有效至 ${signalEnd} · 尚未入场</small></div>${liveSummaryActions}</div>
        <div class="virtual-position-metrics">
          <span><em>当前参考价</em><b>${displayedEntryPrice > 0 ? this.escape(this.compactNumber(displayedEntryPrice)) : "--"}</b><small>实时观察价，尚未冻结入场</small></span>
          <span><em>新闻评分</em><b>${Number.isFinite(displayedNewsScore) ? displayedNewsScore.toFixed(1) : "--"}</b><small>${newsTrigger.version ? `${Number(newsTrigger.new_news_ids?.length || 0)} 条新事件` : "等待新闻快照"}</small></span>
          <span><em>技术评分</em><b>${Number.isFinite(displayedIndicatorScore) ? displayedIndicatorScore.toFixed(1) : "--"}</b><small>${matchedCount} 项指标同向</small></span>
          <span><em>组合评分</em><b>${Number.isFinite(displayedCombinedScore) ? displayedCombinedScore.toFixed(1) : "--"}</b><small>入场阈值 ${Number(item.entry_score_threshold || 80).toFixed(1)}</small></span>
          <span><em>触发进度</em><b>${passedGateCount} / ${signalGateChecks.length}</b><small>${this.escape(waitingDetail || entryState.detail)}</small></span>
          <span><em>信号有效期</em><b>${signalDuration}</b><small>截至 ${signalEnd}</small></span>
        </div>
      </section>` : "";
      ...
      const compactSummaryPanel = positionPanel || candidatePositionPanel || signalSummaryPanel;
      const expandedSignalSummaryPanel = historicalTab ? "" : signalSummaryPanel;
