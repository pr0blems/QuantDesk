...
import json, re, time
from . import store, indicators as ind

# 标的 → 新闻匹配关键词（小写）
KEYWORDS = {
    "NVDA": ["nvidia", "英伟达"], "TSLA": ["tesla", "特斯拉"], "AAPL": ["apple", "苹果"],
    "MSFT": ["microsoft", "微软"], "META": ["meta", "facebook"], "AMD": ["amd"],
    "AMZN": ["amazon", "亚马逊"], "GOOGL": ["google", "alphabet", "谷歌"], "NFLX": ["netflix", "奈飞"],
    "COIN": ["coinbase"], "PLTR": ["palantir"], "MSTR": ["microstrategy", "strategy inc"],
    "TSM": ["tsmc", "台积电"], "BABA": ["alibaba", "阿里巴巴"], "AVGO": ["broadcom", "博通"],
    "ARM": ["arm holdings", " arm "], "ASML": ["asml"], "MU": ["micron", "美光"],
    "INTC": ["intel", "英特尔"], "QCOM": ["qualcomm", "高通"], "TXN": ["texas instruments"],
    "SONY": ["sony", "索尼"], "LLY": ["eli lilly", "礼来"], "JPM": ["jpmorgan", "摩根大通"],
    "GS": ["goldman", "高盛"], "UBER": ["uber", "优步"], "DIS": ["disney", "迪士尼"],
    "WMT": ["walmart", "沃尔玛"], "COST": ["costco"], "CRM": ["salesforce"],
    "CRWD": ["crowdstrike"], "PANW": ["palo alto"], "SNOW": ["snowflake"],
    "CRWV": ["coreweave"], "IREN": ["iren"], "CRCL": ["circle"], "HOOD": ["robinhood"],
    "RKLB": ["rocket lab"], "SNDK": ["sandisk"], "SMCI": ["supermicro", "超微"],
    "DELL": ["dell", "戴尔"], "IBM": ["ibm"], "CSCO": ["cisco", "思科"],
    "ORCL": ["oracle", "甲骨文"], "ADBE": ["adobe"], "PYPL": ["paypal"],
    "PAYP": ["paypal"], "EBAY": ["ebay"], "GME": ["gamestop"],
    "RIVN": ["rivian"], "SOFI": ["sofi"], "DKNG": ["draftkings"],
    "SAMSUNG": ["samsung", "三星"], "SKHYNIX": ["hynix", "海力士"], "SKHY": ["hynix", "海力士"],
    "HYUNDAI": ["hyundai", "现代"], "TENCENT": ["tencent", "腾讯"], "HK0700": ["tencent", "腾讯"],
    "HK1810": ["xiaomi", "小米"], "MINIMAX": ["minimax"], "ZHIPU": ["zhipu", "智谱"],
    "POPMART": ["pop mart", "泡泡玛特"], "OPENAI": ["openai"], "ANTHROPIC": ["anthropic"],
    "XAU": ["gold", "黄金", "金价"], "XAG": ["silver", "白银"], "XPT": ["platinum", "铂金"],
    "XPD": ["palladium", "钯金"], "CL": ["crude oil", " oil ", "原油", "wti"],
    "BZ": ["brent", "布伦特"], "COPPER": ["copper", "铜价", "精铜"], "NATGAS": ["natural gas", "天然气"],
    "SPY": ["s&p 500", "sp500", "标普"], "QQQ": ["nasdaq", "纳指", "纳斯达克"],
    "TQQQ": ["nasdaq", "纳指"], "SQQQ": ["nasdaq", "纳指"], "IWM": ["russell", "罗素"],
    "SOXL": ["semiconductor", "半导体", "chip"], "SOXS": ["semiconductor", "半导体"],
    "SMH": ["semiconductor", "半导体"], "EWJ": ["japan", "日本股市", "日经"],
    "EWY": ["south korea", "韩国股市"], "EWZ": ["brazil", "巴西"], "EWT": ["taiwan", "台湾股市"],
    "KORU": ["south korea", "韩国"], "XLE": ["energy sector", "能源股"],
    "URNM": ["uranium", "铀"], "BITO": ["bitcoin etf", "比特币etf"],
    "UVXY": ["vix", "波动率"], "TMF": ["treasury", "美债", "国债"], "TBT": ["treasury", "美债"],
    "TZA": ["russell", "罗素", "small cap"],
}
MACRO_KEYS = ["federal reserve", "fomc", "cpi", "inflation", "interest rate", "powell", "美联储", "通胀", "非农", "降息", "加息", " fed ", " fed's"]

def conclusion_label(score):
    if score is None: return "数据不足", "#77808f"
    if score >= 75: return "强烈看多", "#2ebd85"
    if score >= 40: return "看多", "#7fc8a9"
    if score <= -75: return "强烈看空", "#f6465d"
    if score <= -40: return "看空", "#e98a97"
    return "中性观望", "#77808f"
...
def market_breadth():
    ...
    if not rows:
        return {"bull": 0, "bear": 0, "neutral": 0, "total": 0, "conclusion": "数据收集中…", "color": "#77808f"}
    ...
    return {"bull": bull, "bear": bear, "neutral": neu, "total": len(rows), "conclusion": c, "color": col}
...
def _tf_scores(symbol):
    ...
    return {r["tf"]: {"score": r["score"], "factors": json.loads(r["detail"] or "[]")} for r in rows}
...
def _match_news(symbol, limit=4, hours=48):
    ...
    rows = store.query("SELECT ts, source, lang, title, title_zh, link, sentiment FROM news WHERE ts>? ORDER BY ts DESC LIMIT 300", (since,))
    hits, macro = [], []
    for r in rows:
...
def build_report(symbol):
    ...
    atr1h = ind.atr([k["high"] for k in kl1h], [k["low"] for k in kl1h], [k["close"] for k in kl1h]) if len(kl1h) > 15 else None
    atr4h = ind.atr([k["high"] for k in kl4h], [k["low"] for k in kl4h], [k["close"] for k in kl4h]) if len(kl4h) > 15 else None
    ...
    if srows:
        ...
        if total > 0:
            ...
            social["stocktwits"] = f"🐂{bull_pct}% vs 🐻{100-bull_pct}%（近{s['st_msgs']}条中{total}条带标签）"
    ...
    if len(kl1h) >= 21:
        ...
        if v_avg > 0:
            r = v_now / v_avg
            stats["1h量比"] = f"{r:.2f} 倍（{'放量' if r >= 1.3 else '缩量' if r <= 0.6 else '正常'}）"
    ...
    def horizon(name, tfs, weights, atr):
        sc_num = sum(tf_data[t]["score"] * wt for t, wt in zip(tfs, weights) if t in tf_data)
        sc_den = sum(wt for t, wt in zip(tfs, weights) if t in tf_data)
        hs = round(sc_num / sc_den) if sc_den else None
        if hs is None:
            return {"name": name, "suggestion": "数据不足", "color": "#77808f",
                    "basis": ["K线数据回填中，暂无法给出该周期建议"], "levels": None, "news": news[:2]}
        if hs >= 40:
            sug, col, side = ("建议做多", "#2ebd85", 1) if hs >= 60 else ("轻仓偏多", "#7fc8a9", 1)
        elif hs <= -40:
            sug, col, side = ("建议做空", "#f6465d", -1) if hs <= -60 else ("轻仓偏空", "#e98a97", -1)
        else:
        ...
        if price and atr and side != 0:
            ...
            levels = {
                "参考入场": fmt(entry),
                "参考止损": fmt(stop) + f"（{side > 0 and '下' or '上'}方 1.5×ATR≈{abs(stop-entry)/entry*100:.2f}%）",
                "参考目标": fmt(target) + f"（{side > 0 and '上' or '下'}方 2.5×ATR≈{abs(target-entry)/entry*100:.2f}%）",
                "盈亏比": "约 1 : 1.67",
            }
        return {"name": name, "score": hs, "suggestion": sug, "color": col,
                "basis": _top_reasons(tf_data, tfs), "levels": levels, "news": news[:3]}
    ...
    return {
        "symbol": symbol, "price": price, "pct_24h": pct,
