...
- P3 Shadow 与恢复：运行链路及 Tiger 闭合 K 线自动增量同步首版已完成，生产观察期尚未开始；
...
- `martingale_tp4` 已由现有独立 Shadow worker 单独分派，不再进入单信号/单仓位求值器；
- Tiger Level-2 盘口仅负责突破与加腿信号，Binance 深度盘口负责虚拟成交、盯市、止盈止损和追踪退出；
- `MaxSpred` 只检查 Binance 可成交点差，不再错误使用 Tiger 参考点差；Tiger 与 Binance 使用各自的价格步长；
- 从证券主数据恢复 Tiger 与 Binance 同一证券关联，未核验映射、缺失 Binance `PRICE_FILTER`、行情过期、跨源时钟偏差或基差超限均只阻止新增风险；
- 从已落库 Tiger 闭合 K 线重建非前视箱体；K 线预热不足或超过两个周期未更新时不创建新篮子；
- 市场 Worker 会从运行中的该策略 Shadow 部署发现所需品种和周期，只为已核验的 Tiger/Binance 主数据关联增量同步；首次预热覆盖完整 999 Bar 箱体搜索及 D1 ATR，后续重叠最后一根 Bar 幂等刷新；
- Tiger 返回的当前未闭合 Bar 在自动同步和手工回填两条入口都会被拒绝入库；同步质量、未核验映射和脱敏失败类别写入 `AdminSetting`，单轮限流避免部署品种较多时冲击官方 API；
- 篮子、逐腿虚拟成交和事件写入现有持久化表，进程重启可恢复活动腿、箱体、前一报价和追踪有利极值；
- deployment 行锁、活动篮子唯一键和事件幂等键共同防止并发重复腿；过期状态转换会被拒绝并要求按最新篮子重新求值；
- Shadow 成交按 Binance bid/ask 加不利滑点和手续费记账，当日已实现亏损、周期亏损、保证金占用、名义价值、加腿开关和估算强平缓冲均在新增风险前检查；
- 数据质量门禁对减仓/退出 fail-open：映射或 Tiger 行情异常不会阻止已有 Binance 虚拟仓位止损；
- 提供按租户隔离的 Shadow 篮子列表与逐腿/事件详情只读接口；所有审计证据明确记录 `network_write=false`；
- 真实 Binance Broker 未接入，当前仍为 Live NO-GO；需先完成至少 20 个交易日 Shadow 观测和故障演练。
