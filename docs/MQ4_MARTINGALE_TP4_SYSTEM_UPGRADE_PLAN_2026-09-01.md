...
> 参数与内核实施基线：`docs/MQ4_MARTINGALE_TP4_CONFIG_ENGINE_DEVELOPMENT_PLAN_2026-09-02.md`（字段语义以此文档为准）
...
- 老虎历史 Bar 确定性回放、逐腿账本、成本模型、区间质量门禁和数据集/运行哈希；
- 单元、策略 API 和前端路由回归测试。

仍未启用：

- Tiger tick/Binance 历史执行价的成交顺序消歧、Shadow、模拟盘和 Binance 实盘执行；
...
> 本节保留早期架构草案供追溯；实际 schema v2 字段及 MQ4 逐项映射以 2026-09-02 参数与内核实施基线为准。
