...
- [x] 定向测试：8 项通过。
- [x] 全量后端测试：1,044 项通过，106 项按环境条件跳过。
...
- [ ] 将 `historical_replay.py` 使用的 `ai_monitor._ablation_signal_state` 提升为稳定应用端口。
- [ ] 增加冻结回放结果测试。
- [ ] 删除跨模块私有调用并建立架构门禁。
...
下一批执行 **C4：历史回放解除私有依赖**。原因是范围最小、风险较低、可增加清晰架构门禁；完成后再继续 **C2-1B** 的兼容观察与删除，避免在同一提交同时碰历史回放和市场特征生产路径。
