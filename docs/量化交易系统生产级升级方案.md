...
- 晋级证据改为 revision 级追加记录：样本外、双倍成本压力、28 天影子、100 笔同执行链模拟、微型实盘与故障演练均有硬门槛，缺任一证据即 fail closed。
- 微型实盘和正式实盘必须先生成晋级申请，再由管理员复核时重新计算门槛并审批；审批不能复用过期 revision。
- 历史版本回滚不覆盖旧记录，而是复制为新的 `draft` 修订并重新执行完整验证流程。
- 增加不可变策略 artifact 与每次 backtest/paper/shadow/live 运行 manifest，记录源码/参数/依赖/引擎/成本/成交/风控版本哈希。
- API、行情、影子、模拟、实盘、AI 与运维监控拆分为独立进程；Worker 使用数据库租约防止同类型重复运行，并持续写入版本化心跳。
- 增加真实持久化 shadow Worker：与模拟/实盘共用策略求值入口，保存稳定 decision ID、信号和无网络写入的订单意图证据。
- 增加全局、账户、revision、品种、数据源和交易通道分层熔断；实盘在每次新增风险前重新读取控制状态，平仓和保护操作不被阻断。
- 增加独立 ops Worker、P0/P1/P2 事故表和 Prometheus 文本指标；Worker 心跳、未知订单意图或实盘 revision 越权会自动告警并冻结对应新增风险，恢复必须人工复核解除熔断。
- 前端新增风险控制台，集中显示 Worker 新鲜度、P0 事故、分层熔断和管理员接管/解决流程。

代码完成不等于生产样本完成。部署后仍必须积累并留档以下运行证据，未满足前系统会自动阻断晋级：

- 至少 28 天真实 shadow 运行与不低于 99.9% 的回放/实时决策一致率。
- 至少 100 笔同执行链模拟成交，并证明双倍成本压力下净收益仍为正。
- 九类关键故障演练全部通过并留档，包括 ACK 前崩溃、部分成交断线、保护单部分失败、用户流缺口、REST 对账失败、数据库提交失败、租约切换和服务重启。
- 旧策略逐一人工复核、重新回测并按 revision 逐级晋级；迁移不会自动继承旧 `published` 权限。
- 现有旧实盘写路径切换到 ExecutionService/MySQL checkpoint 内核前继续视为兼容路径；扩大真实资金必须在该切换、交易所对账演练和人工审批完成之后进行。
